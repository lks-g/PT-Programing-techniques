...
list<int>::const_iterator findValue(const list<int> & data, int value) noexcept {
    return std::find(data.cbegin(), data.cend(), value);
}
...
void helpAfterExam2(list<int> & points) noexcept {
    for(auto& point : points){point = incrementPointsIfItHelps(point);}
}
...
/*
    list<int> newArray(data1.size());

    std::transform(data1.begin(), data1.end(), data2.begin(), newArray.begin(), std::plus<int>());

    return newArray; */

    list<int> out(data1.size());
    std::transform(data1.begin(), data1.end(), data2.begin(), out.begin(), std::plus<int>());
    return out;
}
...
void sort2(vector<int> & data) noexcept {
    std::stable_sort(data.rbegin(), data.rend());
}
...
int main() {
    ...
    // tu mozete doplnit testovaci ko
