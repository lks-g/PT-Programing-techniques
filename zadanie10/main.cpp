...
public:
    ...
    Complex operator+=(const Complex result){
        ...
        this->imaginary = this->imaginary + result.imaginary;
        return *this;
    };
};
...
ostream& operator<<(ostream& stream,const Complex part){

    if(part.getRealPart() > 0 && part.getImaginaryPart() > 0)
    {
        stream << part.getRealPart() << "+" << part.getImaginaryPart() << "i";
    }

    else if(part.getRealPart() > 0 && part.getImaginaryPart() < 0)
    {
        stream << part.getRealPart() << part.getImaginaryPart() << "i";
    }
